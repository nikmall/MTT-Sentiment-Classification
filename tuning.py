...
def objective(trial):
    ...
    param_mtt_fuse = {
        ...
        "enc_heads": trial.suggest_int("enc_heads", 4, 6),
        "dec_heads": trial.suggest_int("dec_heads", 4, 6),
        "enc_pf_dim": trial.suggest_int("enc_pf_dim", 380, 580),
        "dec_pf_dim": trial.suggest_int("dec_pf_dim", 380, 580),
        "enc_dropout": trial.suggest_float("enc_dropout", 0.12, 0.35, step=0.01),
        "dec_dropout": trial.suggest_float("dec_dropout", 0.12, 0.35, step=0.01),
        "att_dropout": trial.suggest_float("att_dropout", 0.10, 0.35, step=0.01),

        "sent_hid_dim": trial.suggest_int("sent_hid_dim", 100, 180),
        "sent_final_hid": trial.suggest_int("sent_final_hid", 80, 148),
        "sent_dropout": trial.suggest_float("sent_dropout", 0.12, 0.35, step=0.01),
        "sent_n_layers": trial.suggest_int("sent_n_layers", 1, 2),
        ...
        "n_epochs": 150,
        'lr_patience': 20,
        'loss_dec_weight': trial.suggest_float("loss_dec_weight", 0.10, 0.17, step=0.01),
        'loss_dec_cycle_weight': trial.suggest_float("loss_dec_cycle_weight", 0.07, 0.12, step=0.01),
